...
Refit scripts (paths in this public repository):
- `scripts/07_refit_family_genus_smoking_ols.py`
- `scripts/08_refit_mediation_panel.py`
...
### Outputs (all under `outputs/` after running script 07)
- `family_smoking_results_corrected.tsv` (replaces `family_smoking_results.tsv` for downstream use; reference copy committed at `outputs-reference/family_smoking_results.tsv`)
- `family_shared_results_corrected.tsv` (smoking-side recomputed; age-side joined from existing CLEAN `family_age_results.tsv`; reference copy committed at `outputs-reference/family_shared_results.tsv`)
- `genus_smoking_results_corrected.tsv` (reference copy at `outputs-reference/genus_smoking_results.tsv`)
- `genus_shared_results_corrected.tsv` (reference copy at `outputs-reference/genus_shared_results.tsv`)
- `supp_family_overlap_scatter.{png,pdf}` (overwrote, identical style)
- `supp_genus_overlap_scatter.{png,pdf}` (overwrote)
- Buggy backups preserved locally (not committed to this repo) as `family_smoking_results_buggy_smoke_binary.tsv`,
...
### Fix
The published `cvrisk.R` script in the original GGMP processing repository (line 19) filters
...
This is intentionally faithful to the published `cvrisk.R` definition
rather than re-defining the smoker indicator (e.g., `former_smoker → 0`),
since the goal of this fix is to make the ASCVD calculation in the
revision analyses match the one used in the published primary analysis.
...
## Files written / overwritten (paths are relative to the repo root)

### Refit scripts (committed to this repo)
- `scripts/07_refit_family_genus_smoking_ols.py`
- `scripts/08_refit_mediation_panel.py`

### Run-time TSV outputs (regenerated by the scripts; reference copies committed under `outputs-reference/`)
- `outputs/family_smoking_results_corrected.tsv`
...
### Markdown summary
- `outputs/mediation_panel_summary_corrected.md` (re-rendered each run)
- `docs/audit/final_bug_fixes_memo.md` (this file; committed)

### Figures (overwritten on each run — buggy backups preserved locally only)
- `outputs/supp_family_overlap_scatter.{png,pdf}` (local buggy backup → `*_buggy_smoke_binary.{png,pdf}`)
- `outputs/supp_genus_overlap_scatter.{png,pdf}` (local buggy backup → `*_buggy_smoke_binary.{png,pdf}`)
- `outputs/supp_mediation_panel.{png,pdf}` (local buggy backup → `*_smoker_buggy.{png,pdf}`)

### Buggy TSV backups (kept locally for diagnostic record; not committed)
